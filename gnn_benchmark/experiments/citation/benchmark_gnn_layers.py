...
from gnn_benchmark.modules.gnn_block_net import GNNBlockNetNode
from gnn_benchmark.tasks import citation_tasks
...
class BenchmarkGNNLayers(BaseBenchmark):
    tasks = {t.__name__: t for t in [
        citation_tasks.CiteseerTask,
        citation_tasks.CoraTask,
        citation_tasks.PubmedTask,
    ]}
    ...
    def build_model(self, feature_channels, target_channels, n_hiddens, n_layers, conv_type, p_dropout, batch_norm, jumping_mode,
        ...
        output_layer = nn.Sequential(
            nn.Linear(blocks[-1].out_channels.node, n_hiddens),
            nn.ReLU(),
            nn.Linear(n_hiddens, target_channels.node)
        )
        model = GNNBlockNetNode(
            encoder=encoder,
            blocks=blocks,
    ...
    def exec_single_run(self, task_name, conv_type, n_hiddens, n_layers, p_dropout, batch_norm,
        ...
        run_results = self.run_and_eval(task, max_epochs, gpus, early_stop_patience=100)
        return run_results
    ...
    def eval_runs(self):
        ...
        analysis = Analysis(self.runs_db, metric_col, metric_comp="max")
